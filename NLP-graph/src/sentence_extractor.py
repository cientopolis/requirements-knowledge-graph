...
def _get_all_objs(v, is_pas):
    ...
    objs = [
        tok for tok in rights if tok.dep_ in OBJECTS or (is_pas and tok.dep_ == "pobj")
    ]
    objs.extend(_get_objs_from_prepositions(rights, is_pas))

    potential_new_verb, potential_new_objs = _get_obj_from_xcomp(rights, is_pas)
    if (
        potential_new_verb is not None
...
# at he moment a sentence is assumed passive if it has an auxpass verb
    ...
    if hasattr(item, "lefts"):
        for part in item.lefts:
            if part.pos_ in BREAKER_POS:
                break
            if part.lower_ not in NEGATIONS and part.dep_ not in AUX_CONTEXT:
                parts.append(part)
