...
class TestOntoscen(unittest.TestCase):
    ...
    def test_detect_actors(self):   
        ...
        self.assertTrue((self.IRI.scenario1, self.hasActor, self.gardener) in self.graph)
    ...
    def test_detect_resources(self):
        ...
        self.assertTrue((self.IRI.scenario1, self.hasResource, self.tomato_plant) in self.graph)
        self.assertTrue((self.IRI.scenario1, self.hasResource, self.pruning_plier) in self.graph)
        self.assertTrue((self.IRI.episode5, self.hasResource, self.pruning_plier) in self.graph)
    ...
    def test_detect_action(self):
        self.assertTrue((self.IRI.episode0, self.hasAction, self.graph.get_individual("interprets")) in self.graph)
        self.assertTrue((self.IRI.episode1, self.hasAction, self.graph.get_individual("prepares")) in self.graph)
        self.assertTrue((self.IRI.episode2, self.hasAction, self.graph.get_individual("applies")) in self.graph)
        self.assertTrue((self.IRI.episode3, self.hasAction, self.graph.get_individual("improves")) in self.graph)
        self.assertTrue((self.IRI.episode4, self.hasAction, self.graph.get_individual("identifies")) in self.graph)
